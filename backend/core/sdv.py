...
def generate_sdv_data_optimized(num_records: int, metadata_dict: Dict[str, Any], seed_tables_dict: Dict[str, Any],
    ...
    try:
        ...
        try:
            ...
            update_progress("processing", "Validating metadata", 10)
            metadata.validate()
        ...
        try:
            ...
            if num_tables == 1 or not has_relationships:
                ...
                for table_name, table_df in seed_tables.items():
                    ...
                    update_progress("processing", f"Training {table_name}", 30)
                    synthesizer.fit(table_df)
                    
                    # Generate data in batches to manage memory while ensuring exact record count
                    update_progress("processing", f"Generating {table_name}", 50)
                ...
                update_progress("processing", "Preparing relational synthesis", 25)
                ...
                update_progress("processing", "Training HMA synthesizer", 30)
                synthesizer.fit(cleaned_tables)
                
                # Generate data in batches with proper scaling
                update_progress("processing", "Generating relational data", 60)
        ...
        update_progress("error", f"Synthesis failed: {str(e)}", 0, error=str(e))
        raise e